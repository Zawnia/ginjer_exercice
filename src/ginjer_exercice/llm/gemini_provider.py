...
class GeminiProvider(LLMProvider):
    ...
    def generate_structured(
        ...
        """Génère une réponse structurée avec retry sur erreur de parsing."""
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                contents.append(types.Content(role="model", parts=[types.Part.from_text(text=(response.text if response else "{}"))]))
                contents.append(types.Content(role="user", parts=[types.Part.from_text(text=error_message)]))
                logger.warning(
                    "Gemini structured output validation failed, retrying (%d/%d): %s",
                    attempt + 1,
                    max_retries + 1,
                    last_error,
                )
